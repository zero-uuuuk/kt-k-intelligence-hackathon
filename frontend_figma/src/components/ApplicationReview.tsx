...
import { Search, FileText, ChevronLeft, ChevronRight, ArrowLeft, ChevronDown, ChevronUp, Sun, Moon, User, Brain, Target, TrendingUp, AlertTriangle, CheckCircle, XCircle, Award, GraduationCap, ShieldCheck, Heart, Zap, BookOpen, Lightbulb, Eye, MessageSquare } from "lucide-react";
...
export function ApplicationReview({ onBack, onFinalEvaluation, currentWorkspaceId, memos, setMemos, applicantStatuses, setApplicantStatuses, getApplicantsByWorkspace }: ApplicationReviewProps) {
  ...
  const convertApplicationsToApplicants = (applications: ApplicationResponseDto[]): Applicant[] => {
    return applications.map(app => ({
      ...
      status: apiUtils.convertApplicationStatus(app.applicationStatus),
  ...
  const [selectedApplicant, setSelectedApplicant] = useState(getDefaultApplicant(currentWorkspaceId));
  ...
  // 평가 기준별 점수 계산 (50점 만점: 학력 10점, 직무경험 8점, 자격증 7점, 봉사시간 10점, 글자수 4점, 자기소개서 11점)
  const getDetailedScores = (applicantName: string) => {
    if (applicantName === '이지호') {
      return {
        education: { score: 8, maxScore: 10, description: '4년제 대학, 컴퓨터공학과, 3.78/4.5' },
        experience: { score: 6, maxScore: 8, description: 'FE 개발 인턴 4개월' },
        certificates: { score: 5, maxScore: 7, description: '웹디자인기능사' },
        volunteer: { score: 7, maxScore: 10, description: '봉사활동 70시간' },
        charCount: { score: 4, maxScore: 4, description: '적정 글자수 유지' },
        essay: { score: 12, maxScore: 11, description: '우수한 자기소개서' }
      };
    }
    
    // 김지원 기본값
    return {
      education: { score: 9, maxScore: 10, description: '4년제 대학, 컴퓨터공학과, 3.96/4.5' },
      experience: { score: 7, maxScore: 8, description: 'BE 팀 인턴 6개월' },
      certificates: { score: 6, maxScore: 7, description: '정보처리기사' },
      volunteer: { score: 8, maxScore: 10, description: '봉사활동 80시간' },
      charCount: { score: 3, maxScore: 4, description: '글자수 약간 부족' },
      essay: { score: 6, maxScore: 11, description: '보통 수준의 자기소개서' }
    };
  };
...
이러한 경험들을 통해 저는 백엔드 개발에 필요한 기술적인 역량뿐만 아니라, 협업을 통해 시너지를 창출하는 자세를 갖추게 되었습니다. 귀사 AI 개발 팀의 백엔드 인턴으로 합류하여, 제가 가진 지식과 경험을 바탕으로 팀의 목표 달성에 기여하고 싶습니다.`,
  ...
  const detailedScores = useMemo(() => {
    return getDetailedScores(selectedApplicant);
  }, [selectedApplicant]);
                  ...
                  <div className="bg-white dark:bg-background border border-gray-200 dark:border-border rounded-lg p-6 mb-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="text-right">
                        ...
                        <div className={`text-2xl font-semibold ${getScoreColor(selectedApplicantData?.score || 0)}`}>
                          {selectedApplicantData?.score}점 / 50점
                        </div>
                      </div>
                    </div>
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-4">
                          <div className="space-y-3">
                            <div className="flex items-center justify-between">
                              <span className="text-sm text-gray-600 dark:text-muted-foreground">학력</span>
                              <span className={`text-sm font-medium ${getScoreColor(detailedScores.education.score)}`}>
                                {detailedScores.education.score}/{detailedScores.education.maxScore}점
                              </span>
                            </div>
                            <div className="flex items-center justify-between">
                              <span className="text-sm text-gray-600 dark:text-muted-foreground">직무 경험</span>
                              <span className={`text-sm font-medium ${getScoreColor(detailedScores.experience.score)}`}>
                                {detailedScores.experience.score}/{detailedScores.experience.maxScore}점
                              </span>
                            </div>
                            <div className="flex items-center justify-between">
                              <span className="text-sm text-gray-600 dark:text-muted-foreground">자격증</span>
                              <span className={`text-sm font-medium ${getScoreColor(detailedScores.certificates.score)}`}>
                                {detailedScores.certificates.score}/{detailedScores.certificates.maxScore}점
                              </span>
                            </div>
                          </div>
                          <div className="space-y-3">
                            <div className="flex items-center justify-between">
                              <span className="text-sm text-gray-600 dark:text-muted-foreground">봉사시간</span>
                              <span className={`text-sm font-medium ${getScoreColor(detailedScores.volunteer.score)}`}>
                                {detailedScores.volunteer.score}/{detailedScores.volunteer.maxScore}점
                              </span>
                            </div>
                            <div className="flex items-center justify-between">
                              <span className="text-sm text-gray-600 dark:text-muted-foreground">글자수</span>
                              <span className={`text-sm font-medium ${getScoreColor(detailedScores.charCount.score)}`}>
                                {detailedScores.charCount.score}/{detailedScores.charCount.maxScore}점
                              </span>
                            </div>
                            <div className="flex items-center justify-between">
                              <span className="text-sm text-gray-600 dark:text-muted-foreground">자기소개서</span>
                              <span className={`text-sm font-medium ${getScoreColor(detailedScores.essay.score)}`}>
                                {detailedScores.essay.score}/{detailedScores.essay.maxScore}점
                              </span>
                            </div>
                          </div>
                        </div>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <div className="font-medium text-gray-900 dark:text-foreground">학력</div>
                                  <div className="text-sm text-gray-600 dark:text-muted-foreground">
                                    {currentQuestionData?.basicInfo?.university} ({currentQuestionData?.basicInfo?.gpa})
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <div className="font-medium text-gray-900 dark:text-foreground">자격증</div>
                                  <div className="text-sm text-gray-600 dark:text-muted-foreground">
                                    {currentQuestionData?.basicInfo?.certificate}
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <div className="font-medium text-gray-900 dark:text-foreground">어학</div>
                                  <div className="text-sm text-gray-600 dark:text-muted-foreground">
                                    {currentQuestionData?.basicInfo?.language}
                                  </div>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="space-y-4">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <div className="font-medium text-gray-900 dark:text-foreground">수상 경력</div>
                                  <div className="text-sm text-gray-600 dark:text-muted-foreground">
                                    {currentQuestionData?.basicInfo?.awards}
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <div className="font-medium text-gray-900 dark:text-foreground">경력</div>
                                  <div className="text-sm text-gray-600 dark:text-muted-foreground">
                                    {currentQuestionData?.basicInfo?.experience}
                                  </div>
                                </div>
                              </div>
                            </div>
